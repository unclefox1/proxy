...
const KUNISHITEI_MAX_ATOMIC_REQUESTS = 180;
...
async function countKunishiteiSegment(segment) {
  const atomics = expandKunishiteiSegment(segment);
  if (atomics.length > KUNISHITEI_MAX_ATOMIC_REQUESTS) {
    throw new Error(`検索条件が多すぎます（${atomics.length}件）。地域・時代を絞ってください。`);
  }
  const session = await createKunishiteiSession();
  let count = 0;
  for (const atomic of atomics) {
    const html = await postKunishiteiForm(session, KUNISHITEI_SEARCH_PATH, buildKunishiteiSearchParams(atomic));
    count += parseKunishiteiSearchCount(html);
  }
...
async function fetchKunishiteiSegmentsCsv(segments) {
  ...
  const atomics = segments.flatMap(expandKunishiteiSegment);
  if (atomics.length > KUNISHITEI_MAX_ATOMIC_REQUESTS) {
  ...
  for (const atomic of atomics) {
    ...
    if (count > KUNISHITEI_MAX_RESULTS) {
      throw new Error(`${atomic.categoryLabel || atomic.categoryId} の一部条件が${KUNISHITEI_MAX_RESULTS}件を超えています。`);
    }
    if (count <= 0) continue;
    totalCount += count;
...
function expandKunishiteiSegment(segment) {
  const categoryId = String(segment.categoryId || "").trim();
  if (!/^\d{3}$/.test(categoryId)) throw new Error("文化財分類IDが不正です。");
  const regionValues = normalizeKunishiteiValues(segment.regionValues);
  const eraValues = normalizeKunishiteiValues(segment.eraValues);
...
function buildKunishiteiSearchParams(atomic) {
...
async function createKunishiteiSession() {
  const response = await fetch(`${KUNISHITEI_BASE_URL}${KUNISHITEI_SEARCH_PATH}`, {
...
async function postKunishiteiForm(session, path, params) {
  ...
  const response = await fetch(`${KUNISHITEI_BASE_URL}${path}`, {
    method: "POST",
    headers: {
...
async function postKunishiteiCsv(session) {
  ...
  params.set("_method", "POST");
  params.set("_csrfToken", session.csrfToken);
  const response = await fetch(`${KUNISHITEI_BASE_URL}${KUNISHITEI_CSV_PATH}`, {
    method: "POST",
    headers: {
      ...kunishiteiOfficialHeaders(),
      "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
...
async function fetchKunishiteiMapChangeGroup(registerId, items) {
  ...
  const response = await fetch(url, { headers: kunishiteiOfficialHeaders() });
